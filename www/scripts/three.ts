...
const MainScene = () => {
  ...
  // add 2d text
  const text = new TextTexture('Flappy Bird', { fontWeight: 'bold', fontSize: 48 })
  ...
  // blue box
  physics.add.box({ x: 0.01, y: 10 }, { lambert: { color: 0x2194ce } })
  ...
  const greenSphere = factory.add.sphere({ y: 2, z: 5 }, { lambert: { color: 0x00ffff } })
